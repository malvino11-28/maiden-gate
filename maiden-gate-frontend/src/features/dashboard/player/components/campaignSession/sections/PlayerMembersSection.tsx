...
export default function PlayerMembersSection({ members }: Props) {
  return (
    <div className="space-y-3">
      {members.length === 0 ? (
        <div className="rounded-xl border border-amber-900/20 bg-slate-900/50 px-5 py-8 text-center text-sm text-amber-100/35">
          Nenhum membro encontrado nesta campanha.
        </div>
      ) : (
        members.map((member) => {
            ...
            <div
              ...
              className={`flex items-center gap-4 rounded-xl border px-5 py-4 ${
                member.voce
                  ...
                  : "border-amber-900/25 bg-slate-900/50"
              }`}
            >
              ...
              <Shield
                className={`h-4 w-4 flex-shrink-0 ${
                  member.voce ? "text-amber-400" : "text-amber-100/20"
                }`}
              />
            </div>
          );
