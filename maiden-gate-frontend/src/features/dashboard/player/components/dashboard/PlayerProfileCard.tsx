...
import { myCampaigns, playerCharacters, profileQuickActions } from "../../data/playerDashboardMock";
...
export default function PlayerProfileCard({ playerName, onChangeTab }: Props) {
  const activeCampaigns = myCampaigns.filter((campaign) => campaign.status === "ativa").length;

  return (
    <div className="max-w-xl space-y-5">
      <h2 className="mb-2 text-lg font-semibold text-amber-100">Informações do Perfil</h2>
      ...
      <div className="rounded-xl border border-rose-900/25 bg-slate-900/50 p-6">
        <div className="space-y-5">
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="text-lg font-semibold text-amber-100">{playerName}</p>
          ...
          <div className="grid gap-4 border-t border-rose-900/20 pt-4 sm:grid-cols-2">
            {[
              ...
              { label: "Personagens criados", value: String(playerCharacters.length) },
          ...
          <div className="flex gap-3 border-t border-rose-900/20 pt-4">
            <Button variant="outline" className="border-rose-500/40 text-rose-400 hover:bg-rose-500/10">
              Editar Perfil
            </Button>
      ...
      <div className="grid grid-cols-2 gap-3 pt-2">
        {profileQuickActions.map(({ icon: Icon, label, tab }) => (
          ...
          >
            ...
            <span className="text-sm text-amber-100/70 group-hover:text-amber-100">{label}</span>
          </button>
        ))}
      </div>
    </div>
