...
export default function CharacterCard({ character }: Props) {
  ...
  return (
    <button
      onClick={() => navigate(`/dashboard/player/character/${character.id}/edit`)}
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${character.marcaCor} text-2xl`}>
            {character.marcaEmoji}
          </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-amber-900/15 pt-4">
        {[
          ...
          { icon: BarChart2, label: "HP", value: `${character.hp}/${character.hpMax}` },
